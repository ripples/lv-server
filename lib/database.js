 ...
 * ]
 */
function getCourseMetaData(courseId, callback) {
  pool.getConnection((err, connection) => {
    if (err) {
      logger.error(err.message);
      callback(err);
    } else {
      connection.query("SELECT c.name as course_name, " +
        ...
        "FROM courses c INNER JOIN " +
          ...
          "WHERE u.user_type_id = 3 AND lcu.course_id = ? " +
        ") prof " +
        "ON prof.course_id = c.id WHERE c.id = ?",
        [courseId, courseId],
        (err, result) => {
          handleResult(err, callback, result);
          connection.release();
