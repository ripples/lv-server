...
/**
 * PS - this function is not finished yet
 * TODO: wirte the query that returns all associated results for the search content i.e.(lectues, courses, etc..)
 * @param searchContent {string} - string with the content to search
 * @param courseId {string} - optional paramenter for the search - case == 0 search in all courses
 * @param callback {function} - callback to execute when the query completes
 */
function getSearchResult(searchContent, courseId = 0, callback) {
  pool.getConnection((err, connection) => {
    if (err) {
      logger.error(err.message);
      callback(err);
